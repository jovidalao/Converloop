...
- 辅助:**Task Agent / 学习项目**(把开放式学习需求变成项目 + 专项课草案)、**Learning Agent / 专项课**(定制化学习会话)、**Explain Agent**(按需,讲解对话回复)、**Reply Suggestion Agent**(按需,推荐用户可发送的地道回复)+ **TTS**(朗读),都不在热路径。见 [architecture](./architecture.md)。
...
- [cross-platform.md](./cross-platform.md) — **跨平台(macOS + Windows)**:双平台开发约定、Windows 适配现状、CI 矩阵
- [craft-ui-plan.md](./craft-ui-plan.md) — **Craft UI/UX 改进计划**:学习 craft-agent 的交互设计、界面设计、侧边栏、Turn Card、输入区、划词浮岛与微交互
- [ui-design-guide.md](./ui-design-guide.md) — **UI 文字与颜色指南**:Craft 风格字号梯度、文字灰度、语义色与验收检查
...
- [reply-suggestion-agent.md](./reply-suggestion-agent.md) — 按需推荐回复:用户消息改写 / AI 回复续答
