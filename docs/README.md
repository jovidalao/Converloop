...
AI 语言学习 agent。本文件是入口,详细设计见同目录其他文档。
...
- **不假设本地 LLM**:现实默认是 BYOK 托管模型(OpenAI/Claude/OpenRouter);本地模型是可选高级功能。
...
- **Tutor Agent**(每轮,热,与上者并行)— 读 SQLite 薄弱表 + 输入,输出结构化 `TutorAnalysis`(纠错 + 掌握信号)。
- **Profile Maintainer Agent**(偶尔,后台)— 读现有 MD + SQLite 聚合 + 近期对话,重写 MD 档案。

## 两层存储(核心决策)

- **SQLite = 地面真相**:每个掌握项的计数/状态,**代码**每轮从信号派生维护。确定性、可排序、可画进度。LLM 不碰计数。
- **MD 档案 = 叙述层**:定性人设(在练什么/已掌握/回避/兴趣/最近学到),**维护 agent** 偶尔重写。人类可读可编,直接喂对话 agent。

> 对话 agent 读 MD,导师 agent 读 SQLite;代码写 SQLite,维护 agent 写 MD。

## 核心原则

- **LLM 观察,代码记账**:LLM 给离散信号(error/correct/introduced),计数和置信度由代码算。
- **mastery_key 稳定**:同一类错误永远同一个 key,这是掌握系统的地基。
- **热路径便宜确定**,贵的 LLM 维护放后台批量跑。

## 开工顺序(别反)

1. 半天,不写代码:真实句子手动验证三个 prompt,盯 `mastery_key` 跨句稳定性。
2. 第一周第一个编码任务 = 技术探针:Tauri webview 里 Drizzle + SQLite 读写 + migration(不开箱即用)。
3. 密钥存设备绑定加密文件(应用自管,无主密码,见 `src-tauri/src/secrets.rs`),绝不明文。
4. 然后正式开发。

## 文档

- [architecture.md](./architecture.md) — 数据流、存储、schema、SQL、缓存
- [conversation-agent.md](./conversation-agent.md)
- [tutor-agent.md](./tutor-agent.md)
- [profile-maintainer-agent.md](./profile-maintainer-agent.md)
