...
实现按**稳定优先拆成 3 条 system 消息**(`src/agents/learning.ts` 的 `systemMessages`):①基础规则(只依赖语言配置)②偏好 + 课程 prompt(课程内稳定)③数据上下文 + 滚动摘要(动态)。Anthropic 适配器给除最后一块外的块打缓存断点;OpenAI 兼容线格式重新合并成单条 system。下面是合并后的逻辑视图:
...
课堂历史作为真实 user/assistant 交替消息发送(`buildHistoryMessages`),滚动摘要(`STORY SO FAR`)在 system 块 3 末尾;最后一条 user 消息只含本轮学习者输入(或开课 kickoff 指令)。
